# Governance

**Status:** Current  
**Owner:** Architecture (TBD)  
**Last Reviewed:** 2026-08-13  

| Doc | Purpose |
| --- | --- |
| [Architecture Portal v1](architecture-v1.md) | What v1 includes / does not finalise |
| [Change process](architecture-change-process.md) | When and how to change architecture |
| [Change checklist](architecture-change-checklist.md) | PR checklist |
| [Review cadence](review-cadence.md) | Event / pre-pilot / pre-prod / periodic |
| [Document metadata](document-metadata.md) | Header convention |
| [CODEOWNERS guidance](codeowners-guidance.md) | Ownership areas (names TBD) |
| [ADR template](../decisions/ADR-TEMPLATE.md) | New ADR structure |
