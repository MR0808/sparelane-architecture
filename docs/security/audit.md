...
**H1:** privileged grant request/approve/deny/execute/fail must emit durable audit per [ADR-033](../decisions/ADR-033-privileged-admin-grant-management-and-approval.md). No financial mutation audit from admin in H1 — grant management only.
...
- **privileged admin mutations** — H1 grant catalogue below; other mutations deferred H2+
...
- manual replay / retry actions by authorised operators (H2+)
