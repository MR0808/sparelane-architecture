## Change Summary

<!-- What changed and why (1–3 bullets). -->

## Architecture Impact

<!-- Views/model/flows touched, or "docs only". -->

## Decision Impact

<!-- New/updated ADR? Open decisions changed? None? -->

## Contracts/Data Impact

<!-- OpenAPI, webhooks, schema/ERD — or none. -->

## Security/Privacy Impact

<!-- Trust boundary, PCI, secrets, tenant isolation — or none. -->

## Validation

- [ ] LikeC4 validates (`npm run architecture:validate`)
- [ ] LikeC4 builds (`npm run build`)
- [ ] OpenAPI lint passes (`npm run openapi:lint`)
- [ ] Doc links pass (`npm run docs:links`)
- [ ] ADR updated/added if needed
- [ ] Dynamic flows updated if behaviour changed
- [ ] Open decisions updated if relevant
